...
# would've taken the seal to descend to depth, assuming the seal is descending 
# at 1.1 m/s (from Williams et al., 2015). The time of this descent was then 
# used to transform x2 over in the -x direction, such that the two points 
# would be along the same circle with a center of (h, k). Then, the center of 
# the circle was calculated using methods described in: 
# 
...
find_center_y <- function(x1, y1, x2, y2, r = 20.87, rate, psi_calibration = psi_calibration){
  ...
  # using psi and cm of this psi calibration to calculate depth 
