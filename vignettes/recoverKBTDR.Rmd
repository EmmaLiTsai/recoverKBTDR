...
## Step 2: Arc Removal 
### Remove left-leaning arc from the records: 
...
## Step 4: X-axis transformation to dates & times 
This step can be completed in two parts. The first function transform_x_vals() and uses the timing dots to create a time scale. The x-axis is also assigned a time, which can be transformed to a POSIXct object. 
...
```{r}
# Transform the x-axis to time in minutes from the origin. The center_y value is an estimated height of the transducer arm pivot point in centimeters. In other words, it is the height of the circle that the transducer arm makes. 
