...
After centering, you can see how the original record was shifted down such that y = 0 aligns better with surface values. This function also adds the centered psi calibration curve to the global environment, if present in the record:
```{r}
df <- center_scan(trace, time_dots, 0.9,                     psi_interval = c(100, 200, 400, 600, 800))
...
## Step 2: Arc Removal  
The KBTDR documented the diving behavior of an animal by using a pressure sensitive transducer arm that rotated an LED light along a rolling spool of film. From this design, the original records exhibit a characteristic left leaning arc. This function transform_x_vals() removes the arc in the record (new_x) and also uses the timing dots to create a time scale. Helper functions are also available to estimate center_y. For more information, type ?find_center_y()

```{r}
# removing the data frame and also transform the x-axis to time in minutes from the origin. The center_y value is an estimated height of the transducer arm pivot point in centimeters. 
...
ggplot2::ggplot(df[1000:11000,], aes(x = x_val, y = y_val)) + 
  geom_point() + 
  geom_point(aes(x = new_x, y = y_val), color = "#39b3b2")
```
...
The seal often moved faster than the LED arm could document the dive during the descent and ascent phases. As a result, the KBTDR records often have gaps between the beginning of the dive and the bottom phase. The function add_dates_times() uses the trace data frame to add a POSIXct date time object, and also interpolates between missing values to create a regular time series. Creating a regular time series wouldn't just benefit future spline smoothing, but is also a requirement for dive analysis packages. 
...
ggplot(df[1000:18000,], aes(x = date_time, y = depth)) + geom_point() + geom_point(aes(y = smooth_depth), color = "#39b3b2", size = 0.8)
```
